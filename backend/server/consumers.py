...
class ServerConsumer(WebsocketConsumer):
    def connect(self):
        ...
        If a 'guest' query param is present, join immediately as Guest.
        Otherwise, accept connection but wait for an 'auth' message to join.
        """
        ...
        if guest_name:
            ...
            # Unique ID: channel_name + guest_name
    ...
    def receive(self, text_data=None, bytes_data=None):
        ...
        try:
            ...
            if bytes_data:
                # Only forward binary data if authenticated
                if not self.is_authenticated_context:
                    self.send_error("Authentication required")
                    self.close()
                    return
                
                # Forward binary chunk to all other users in the room
    ...
    def _authenticate(self, token):
        ...
        try:
            ...
            self.user = user
            # Unique ID: channel_name + username
    ...
    def _handle_copy_message(self, data):
        ...
        event = {
            ...
            'file_name': file_name,
            'f_user': self.display_name, # Stored as UNIQUE name
            'sender_channel_name': self.channel_name, 
        }
    ...
    def group_copy_handler(self, event):
        ...
        # Sanitize f_user before sending
    ...
    def group_binary_handler(self, event):
        ...
        # self.send() with bytes=... sends a binary WebSocket frame
        self.send(bytes_data=event['bytes_data'])

    def group_user_list_handler(self, event):
        raw_list_str = event['list']
        # Convert string representation back to list to process it
        try:
            import ast
