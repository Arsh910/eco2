    ...
    useEffect(() => {
        ...
        loader.load('/eco2/eco2.glb',
            async (gltf) => {
                ...
                // 2. Initialize PromoManager taking the registry
                promoManager = new PromoManager(registry);

                // 3. Initial Ad Load
                await promoManager.loadAds();

                // 4. Setup 60-second ad refresh schedule per requirements
                adRefreshInterval = setInterval(() => {
                    //console.log('[PromoManager] Normal refresh interval triggered.');
                    promoManager.loadAds();
                }, 60000);
            },
            undefined,
        ...
        return () => {
            ...
            if (promoManager) promoManager.dispose();
            controls.dispose();
            renderer.dispose();
